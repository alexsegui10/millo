...
export function DashboardPage() {
    ...
    const loadDashboard = async () => {
        setLoading(true);
        try {
            const res = await api.get<{ data: DashboardData }>('/dashboard/summary');
    ...
    if (!data) {
        return <div className="p-8 text-center text-gray-500">Error al cargar el panel de control</div>;
    }
    ...
    return (
        <div className="p-8 hide-scrollbar">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <StatCard label="Active Models" value={data.kpis.activeModels.toString()} icon="person" />
                <StatCard label="Active Niches" value={data.kpis.activeNiches.toString()} icon="category" />
                <StatCard label="Posts (Last 7d)" value={data.kpis.postsLast7Days.toString()} icon="post_add" />
                <StatCard
                    label="Followers Growth (7d)"
            ...
            {(data.alerts.scheduledToday > 0 || data.alerts.nichesWithoutMetrics > 0) && (
                <div className="bg-white dark:bg-[#1f2937] rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 mb-8">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Alerts</h2>
                    <div className="space-y-3">
                        {data.alerts.scheduledToday > 0 && (
                            <div className="flex items-center gap-3 text-sm">
                                <span className="material-symbols-outlined text-orange-500">schedule</span>
                                <span className="text-gray-700 dark:text-gray-300">
                                    <strong>{data.alerts.scheduledToday}</strong> posts scheduled for today
                                </span>
                            </div>
                        )}
                        {data.alerts.nichesWithoutMetrics > 0 && (
                            <div className="flex items-center gap-3 text-sm">
                                ...
                                <span className="text-gray-700 dark:text-gray-300">
                                    <strong>{data.alerts.nichesWithoutMetrics}</strong> niches without metrics (last 7 days)
                                </span>
                            </div>
                        )}
                    </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
                                <tr>
                                    ...
                                    <th className="py-3 px-6 text-left font-semibold text-gray-700 dark:text-gray-300">Hook</th>
                                    <th className="py-3 px-6 text-left font-semibold text-gray-700 dark:text-gray-300">Niche</th>
                                    <th className="py-3 px-6 text-left font-semibold text-gray-700 dark:text-gray-300">Posted</th>
            ...
            {data.topPosts.length > 0 && (
                <div className="bg-white dark:bg-[#1f2937] rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                    <div className="p-6 border-b border-gray-100 dark:border-gray-800">
                        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Top Posts (by Saves Rate)</h2>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
                                <tr>
                                    <th className="py-3 px-6 text-left font-semibold text-gray-700 dark:text-gray-300">Type</th>
                                    <th className="py-3 px-6 text-left font-semibold text-gray-700 dark:text-gray-300">Hook</th>
                                    <th className="py-3 px-6 text-right font-semibold text-gray-700 dark:text-gray-300">Saves Rate</th>
                                    <th className="py-3 px-6 text-right font-semibold text-gray-700 dark:text-gray-300">Views</th>
                                    <th className="py-3 px-6 text-right font-semibold text-gray-700 dark:text-gray-300">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            {!data.latestPosts.length && !data.topPosts.length && (
                <div className="bg-white dark:bg-[#1f2937] rounded-xl p-8 shadow-sm border border-gray-100 dark:border-gray-800 text-center">
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Welcome to OFM Agency Hub</h2>
                    <p className="text-gray-600 dark:text-gray-400">
                        {data.kpis.activeModels > 0
                            ? `You have ${data.kpis.activeModels} active model${data.kpis.activeModels > 1 ? 's' : ''} and ${data.kpis.activeNiches} active niche${data.kpis.activeNiches > 1 ? 's' : ''}. Start creating posts!`
